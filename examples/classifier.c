...
void validate_classifier_single16(char *datacfg, char *filename, char *weightfile, int gemmtype)
{
    int i, j;
    int base, range, init;
    char ct;
    float avg_acc = 0;
    float avg_topk = 0;
    FLT *X = calloc(200000, sizeof(FLT));
    if (!X)
    {
    ...
    if (gemmtype == 2 || gemmtype == 82 || gemmtype == 92) //mac2
        ...
        net->gemm_type = 0; //FLT
    /*for(i=0;i< net->n;i++){
        layer16 l = net->layers++;
        l->delta = 0;
        printf("net.layer[%d].delta:%lf,delta[0]:%f\n",i,*(double *)net->layers[i].delta,(float)net->layers[i].delta[0]);
        network16 net1 = *net;
        layer16 l = net1.layers[i];
        printf("net1.layer[%d].delta:%lf,delta[0]%f\n",i,*(double *)l.delta,(float)l.delta[0]);
     }*/
    set_batch_network16(net, 1);
    srand(time(0));
    printf("list *options = read_data_cfg(datacfg)\n");
    ...
    */
    ...
    int fails = 0; //liuj20180719 fail to open image

    clock_t time;
    //int base = 45000, range =5000;
    if (!base || base < 0 || base > m)
        base = 0;
    if (init > base)
        init = base;
    if (!range || range < 0 || range < base || range > m)
        range = m;
    //for(i = 0; i < m; ++i){
    for (i = base; i < range; ++i)
    {
        ...
        //show_image(crop, "cropped");
        //cvWaitKey(0);
        printf("crop.data[0]:%f,crop.data[%d]:%d,%f\n", (float)crop.data[0], net->w * net->h * im.c - 1, crop.w * crop.h * crop.c, (float)crop.data[net->w * net->h * im.c - 1]);

        int count;
        for (count = 0; count < crop.w * crop.h * crop.c; count++)
            X[count] = (FLT)crop.data[count];

        printf("FLT crop.data[0]:%f,crop.data[%d]:%d, %f\n", (float)X[0], net->w * net->h * im.c - 1, crop.w * crop.h * crop.c, (float)X[crop.w * crop.h * crop.c - 1]);

        FLT *pred = network_predict16(net, X);
        if (net->hierarchy)
        {
            printf("hierarchy\n");
            hierarchy_predictions16(pred, net->outputs, net->hierarchy, 1, 1);
        }
        ...
        free_image(crop);
        printf("call top_k16\n");
        top_k16(pred, classes, topk, indexes);

        printf("\n !!!!out top_k16 class:%d,topk:%d\n", class, topk);
        if (indexes[0] == class)
        {
            avg_acc += 1;
            printf("avg_acc:%f\n", avg_acc);
        ...
        printf("%lf seconds \n", sec(clock() - time));
        printf("avg_acc:%f,avg_topk:%f\n", avg_acc, avg_topk);
        printf("%s,class: %d, indexes[0]:%d,indexes[1]: %d, indexes[2]:%d, indexes[3]:%d, indexes[4]%d\n", paths[i], class, indexes[0], indexes[1], indexes[2], indexes[3], indexes[4]);
        printf("image No %d, %d-%d  top 1: %f, top %d: %f\n", i, init, range - 1, avg_acc / (i + 1 - fails - init), topk, avg_topk / (i + 1 - fails - init));
        printf("filed open:%d\n", fails);
        printf("\n");
        printf("\n");
        // printf("%s,class: %d, indexes[0]:%d, %f, %f \n", paths[i], class, indexes[0], pred[0], pred[1]);
    }
}

void validate_classifier_single(char *datacfg, char *filename, char *weightfile, int gemmtype)
{
...
void predict_classifier16(char *datacfg, char *cfgfile, char *weightfile, char *filename, int top)
{
    printf("predict_classifier16\n");
    ...
    while (1)
    {
        ...
        printf("network_predict16 over !\n");
        if (net->hierarchy)
            hierarchy_predictions16(predictions, net->outputs, net->hierarchy, 1, 1);
        top_k16(predictions, net->outputs, top, indexes);
