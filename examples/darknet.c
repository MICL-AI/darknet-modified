...
extern void test_one_convolutional_layer(char *datacfg, char *cfgfile, char *weightfile, char *filename);
...
{
    #ifdef PRUNE conv_reduce_min = 1.111; 
    #endif
    if (argc < 2)
    {
    ...
    //TL 181015 shorting the command for quick testing
    if (0 == strcmp(argv[1], "TL"))
    {
        char net_cfg[60] = "cfg/";
        char net_weights[60] = "weights/";
        char net_name[60], class_type[60], data_path[100];

        //reset argc
        argc = 7;

        //get type, net_name, datapath
        strcpy(class_type, argv[2]);
        strcpy(net_name, argv[3]);
        strcpy(data_path, argv[4] ? argv[4] : "data/dog.jpg"); //default:dog
        ...
        strcat(argv[4], ".cfg");
        //set weiths path
        argv[5] = strcat(net_weights, net_name);
        strcat(argv[5], ".weights");
        //set datapath
        strcpy(argv[6], data_path);
    }
//
#ifndef GPU
    gpu_index = -1;
    ...
    else if (0 == strcmp(argv[1], "oneconvtest"))
    {
        test_one_convolutional_layer("cfg/imagenet1k.data", argv[2], argv[3], argv[4]);
    }
    else
    {
