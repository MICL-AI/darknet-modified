...
/*static void increment_layer16(layer16 *l, int steps)
{
...
{
    fprintf(stderr, "LSTMCJ16 Layer: %d inputs, %d outputs\n", inputs, outputs);
    batch = batch / steps;
    layer16 l = { 0 };
    l.batch = batch;
    l.type = LSTMCJ;
    ...
    l.wf = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.wf)= make_connected_layer16(batch*steps, inputs, outputs,LOGISTIC , batch_normalize, adam);
    l.wf->batch = batch;

    l.wi = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.wi) = make_connected_layer16(batch*steps, inputs, outputs, LOGISTIC, batch_normalize, adam);
    l.wi->batch = batch;

    l.wo = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.wo) = make_connected_layer16(batch*steps, inputs, outputs, LOGISTIC, batch_normalize, adam);
    l.wo->batch = batch;

    l.wc = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.wc) = make_connected_layer16(batch*steps, inputs, outputs,TANH , batch_normalize, adam);
    l.wc->batch = batch;
    

    l.wy = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.wy) = make_connected_layer16(batch*steps, outputs, inputs-outputs, LINEAR, batch_normalize, adam);
    l.wy->batch = batch;

    l.outputs = outputs;
    l.output = calloc(outputs*batch*steps, sizeof(FLT));
    l.state = calloc(outputs*batch, sizeof(FLT));
    l.wo->temp_cpu = calloc(inputs, sizeof(FLT));
    l.wf->temp_cpu = calloc(outputs, sizeof(FLT));
    l.wi->temp_cpu = calloc(outputs, sizeof(FLT));
    l.wc->temp_cpu = calloc(outputs, sizeof(FLT));
    l.c_old = 	calloc(batch*outputs, sizeof(FLT));
    l.h_old = 	calloc(batch*outputs, sizeof(FLT));
    l.probs = 	calloc(batch*(inputs-outputs), sizeof(FLT));
    l.cell_cpu =        calloc(batch*outputs*steps, sizeof(FLT));
    l.f_cpu =           calloc(batch*outputs, sizeof(FLT));
    l.i_cpu =           calloc(batch*outputs, sizeof(FLT));
    l.g_cpu =           calloc(batch*outputs, sizeof(FLT));
    l.o_cpu =           calloc(batch*outputs, sizeof(FLT));
    l.c_cpu =           calloc(batch*outputs, sizeof(FLT));
    l.h_cpu =           calloc(batch*outputs, sizeof(FLT));
    l.temp_cpu =        calloc(batch*outputs, sizeof(FLT));
    l.temp2_cpu =       calloc(batch*outputs, sizeof(FLT));
    l.temp3_cpu =       calloc(batch*outputs, sizeof(FLT));

    l.forward = forward_lstm_layerCJ16;
...
layer16 make_lstm_layer16(int batch, int inputs, int outputs, int steps, int batch_normalize, int adam)
{
    ...
    l.uf = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.uf) = make_connected_layer16(batch*steps, inputs, outputs, LINEAR, batch_normalize, adam);
    l.uf->batch = batch;

    l.ui = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.ui) = make_connected_layer16(batch*steps, inputs, outputs, LINEAR, batch_normalize, adam);
    l.ui->batch = batch;

    l.ug = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.ug) = make_connected_layer16(batch*steps, inputs, outputs, LINEAR, batch_normalize, adam);
    l.ug->batch = batch;

    l.uo = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.uo) = make_connected_layer16(batch*steps, inputs, outputs, LINEAR, batch_normalize, adam);
    l.uo->batch = batch;

    l.wf = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.wf) = make_connected_layer16(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    l.wf->batch = batch;

    l.wi = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.wi) = make_connected_layer16(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    l.wi->batch = batch;

    l.wg = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.wg) = make_connected_layer16(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    l.wg->batch = batch;

    l.wo = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.wo) = make_connected_layer16(batch*steps, outputs, outputs, LINEAR, batch_normalize, adam);
    l.wo->batch = batch;
    ...
    l.forward = forward_lstm_layer16; 
    l.cell_cpu =        calloc(batch*outputs*steps, sizeof(FLT));
    l.f_cpu =           calloc(batch*outputs, sizeof(FLT));
    l.i_cpu =           calloc(batch*outputs, sizeof(FLT));
    l.g_cpu =           calloc(batch*outputs, sizeof(FLT));
...
int g_lstm=0;
char filename[64];
void savereslstm( char * filename,FLT *data, int len)
{
...
void savereslstmf( char * filename,float *data, int len)
{
	FILE *fp = fopen(filename ,"w+");
	int num;
	for(num=0;num<len;num++){
...
void softmaxCJ16(float *input, int n,  float *output)
{printf("sftmaxcj16 float\n");
    int i;
    float sum = 0;
    float largest = -60000;
    for(i = 0; i < n; ++i){
...
clock_t aa,bb;
/*void lstm_vec(layer16 l, network16 state)
{
	...
	while ( i < l.inputs ) {
		if ( i <l.outputs ) {
			...
			s.input[i] =state.input[i-l.outputs]; 
		}
		++i;
	}
	forward_connected_layer16(wf, s);
	forward_connected_layer16(wi, s);
	forward_connected_layer16(wo, s);
	forward_connected_layer16(wc, s);

	copy_cpu16vec(l.outputs*l.batch, wf.output , wf.temp_cpu);	

	mul_or_add_cpu16vec (l.outputs*l.batch, l.c_cpu,   wf.temp_cpu,0);//mul
	copy_cpu16vec(l.outputs*l.batch, wi.output,   wi.temp_cpu);
	mul_or_add_cpu16vec (l.outputs*l.batch, wi.temp_cpu, wc.output ,0);//mul
	mul_or_add_cpu16vec(l.outputs*l.batch,  wf.temp_cpu, wc.output, 1);//add
	copy_cpu16vec(l.outputs*l.batch, wc.output,  l.c_cpu);//l.c_cpu (c_old)

	// h= wo.out *tanh(c)
	activate_array16(wc.output, l.outputs*l.batch, TANH); 
	mul_or_add_cpu16vec(l.outputs*l.batch,wc.output, wo.output,0);//mul
	copy_cpu16vec(l.outputs*l.batch, wo.output, l.h_cpu);//l.h_cpu (h_old)


	s_conn.input=wo.output;
	s_conn.flag_vec = state.flag_vec;
	forward_connected_layer16(wy, s_conn);
	copy_cpu16vec(l.outputs*l.batch, wy.output, l.output);	//output
...
void lstm_scalar(layer16 l, network16 state)
{
	layer16 wf = *(l.wf);
	layer16 wi = *(l.wi);
	layer16 wo = *(l.wo);
	...
	s.flag_vec = state.flag_vec;
       	int i=0;
	FLT *h_old ,*c_old,*input; 
	
	c_old = l.c_cpu;
	h_old = l.h_cpu;
	input =state.input;   

	s.input=wo.temp_cpu;//initial
	while ( i < l.inputs ) {
		if ( i <l.outputs ) {
			s.input[i] =h_old[i];
		} else {
			s.input[i] = input[i-l.outputs];
		}
		++i;
	}
	forward_connected_layer16(wf, s);
	...
	// c = wf.out * c_old + wi.out * wc.out
	copy_cpu16(l.outputs*l.batch, wf.output,  1, wf.temp_cpu, 1);	
	mul_cpu16 (l.outputs*l.batch, c_old, 	  1, wf.temp_cpu, 1);
	
	copy_cpu16(l.outputs*l.batch, wi.output,  1, wi.temp_cpu, 1);
	mul_cpu16 (l.outputs*l.batch, wi.temp_cpu,1, wc.output  , 1);	

	axpy_cpu16(l.outputs*l.batch, 1, wf.temp_cpu, 1,wc.output, 1);	
	copy_cpu16(l.outputs*l.batch, wc.output,  1, l.c_cpu, 1);//l.c_cpu (c_old)
	// h= wo.out *tanh(c)
	activate_array16(wc.output, l.outputs*l.batch, TANH); 
	mul_cpu16(l.outputs*l.batch,wc.output,1, wo.output, 1);	
	copy_cpu16(l.outputs*l.batch, wo.output, 1, l.h_cpu, 1);//l.h_cpu (h_old)  

	s_conn.input=wo.output;
	s_conn.flag_vec = state.flag_vec;
	forward_connected_layer16(wy, s_conn);
	copy_cpu16(l.outputs*l.batch, wy.output, 1, l.output, 1);//output
...
void forward_lstm_layerCJ16(layer16 l, network16 state)
{
	if(state.flag_vec ==0) {
		lstm_scalar(l, state);
	}
...
void forward_lstm_layer16(layer16 l, network16 state)
{
    network16 s = { 0 };
    int i;
    layer16 wf = *(l.wf);
    ...
    for (i = 0; i < l.steps; ++i) {
        ...
        forward_connected_layer16(wf, s);
        forward_connected_layer16(wi, s);
        forward_connected_layer16(wg, s);
        forward_connected_layer16(wo, s);

        s.input = state.input;

        forward_connected_layer16(uf, s);
        forward_connected_layer16(ui, s);
        forward_connected_layer16(ug, s);        
	forward_connected_layer16(uo, s);	

        copy_cpu16(l.outputs*l.batch, wf.output, 1, l.f_cpu, 1);
        axpy_cpu16(l.outputs*l.batch, 1, uf.output, 1, l.f_cpu, 1);

        copy_cpu16(l.outputs*l.batch, wi.output, 1, l.i_cpu, 1);	
        axpy_cpu16(l.outputs*l.batch, 1, ui.output, 1, l.i_cpu, 1);	

        copy_cpu16(l.outputs*l.batch, wg.output, 1, l.g_cpu, 1);	
        axpy_cpu16(l.outputs*l.batch, 1, ug.output, 1, l.g_cpu, 1);	

        copy_cpu16(l.outputs*l.batch, wo.output, 1, l.o_cpu, 1);	
        axpy_cpu16(l.outputs*l.batch, 1, uo.output, 1, l.o_cpu, 1);	

        activate_array16(l.f_cpu, l.outputs*l.batch, LOGISTIC);		
        activate_array16(l.i_cpu, l.outputs*l.batch, LOGISTIC);		
        activate_array16(l.g_cpu, l.outputs*l.batch, TANH);			
        activate_array16(l.o_cpu, l.outputs*l.batch, LOGISTIC);		

        copy_cpu16(l.outputs*l.batch, l.i_cpu, 1, l.temp_cpu, 1);		
        mul_cpu16(l.outputs*l.batch, l.g_cpu, 1, l.temp_cpu, 1);		
        mul_cpu16(l.outputs*l.batch, l.f_cpu, 1, l.c_cpu, 1);			
        axpy_cpu16(l.outputs*l.batch, 1, l.temp_cpu, 1, l.c_cpu, 1);	

        copy_cpu16(l.outputs*l.batch, l.c_cpu, 1, l.h_cpu, 1);		 	
        activate_array16(l.h_cpu, l.outputs*l.batch, TANH);		
        mul_cpu16(l.outputs*l.batch, l.o_cpu, 1, l.h_cpu, 1);	

        copy_cpu16(l.outputs*l.batch, l.c_cpu, 1, l.cell_cpu, 1);		
        copy_cpu16(l.outputs*l.batch, l.h_cpu, 1, l.output, 1);
