...
extern void forward_maxpool_layer(const maxpool_layer l,network net);
...
void savew( char * filename,float *data, int len)
{
...
{
    network16 *net16 = parse_network_cfg_CJ16(cfg);
    if(weights && weights[0] != 0){
        load_weights16(net16, weights);
    }
...
{
    network16 *net16 = parse_network_cfg16(cfg);
    if(weights && weights[0] != 0){
        load_weights16(net16, weights);
    }
...
network16 *make_network16(int n)
{
    network16 *net = calloc(1, sizeof(network16));
    net->n = n;
    net->layers = calloc(net->n, sizeof(layer16));
...
void forward_network(network *netp)
{
    ...
    network net = *netp;
    int i;
    for(i = 0; i < net.n; ++i){ //net.n
        net.index = i;
        layer l = net.layers[i];
        if(l.delta){//printf("l.delta\n"); 
...
void forward_networkCJ16(network16 *netp)
{
    network16 net = *netp;
    int i;
    for(i = 0; i < net.n; ++i){
...
void forward_network16(network16 *netp)
{
    network16 net = *netp;
    int i;  printf("net.n:%d\n",net.n); //printf("net.layers[%d].delta[0]:%f,delta[0]:%f\n",net.n-1,(float)net.layers[net.n-1].delta[0],(float)net.layers[0].delta[0]); 
    for(i = 0; i < net.n; ++i){
        //net.index = i;
        layer16 l = net.layers[i];
...
{   
    printf("network_predict16\n");
    network16 orig = *net;
    net->input = input; printf("net->input[0]%f,net->input[1]:%f,net->input[2]:%f\n",(float)net->input[0],(float)net->input[1],(float)net->input[2]);
    net->truth = 0;
    net->delta = 0;//printf("net->delta:%lf\n",*(double * )net->delta);    
    forward_network16(net);
    FLT *out = net->output;
    *net = orig;
    return out;
}
