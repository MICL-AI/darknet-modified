...
{
    int num = l->outputs*l->batch*steps;//printf("increment_layer: num=%d \n",num);
    l->output += num;
    l->delta += num;
    l->x += num;
    l->x_norm += num;
...
#endif
#endif
printf("make_rnn_layer:input& output size:%d_%d\n",inputs, outputs);
    return l;
}

layer16 make_rnn_layer16(int batch, int inputs, int outputs, int steps, ACTIVATION activation, int batch_normalize, int adam)
{
    fprintf(stderr, "RNN Layer: %d inputs, %d outputs\n", inputs, outputs);
    ...
    l.state = calloc(batch*outputs, sizeof(FLT));

    l.input_layer = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.input_layer) = make_connected_layer16(batch*steps, inputs, outputs, activation, batch_normalize, adam);
    l.input_layer->batch = batch;

    l.self_layer = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.self_layer) = make_connected_layer16(batch*steps, outputs, outputs, activation, batch_normalize, adam);
    l.self_layer->batch = batch;

    l.output_layer = malloc(sizeof(layer16));
    fprintf(stderr, "\t\t");
    *(l.output_layer) = make_connected_layer16(batch*steps, outputs, outputs, activation, batch_normalize, adam);
    l.output_layer->batch = batch;
...
void forward_rnn_layer16(layer16 l, network16 net)
{
    network16 s = net;
    int i;
    layer16 input_layer = *(l.input_layer);
    layer16 self_layer = *(l.self_layer);
    layer16 output_layer = *(l.output_layer);

    fill_cpu16(l.outputs * l.batch * l.steps, 0, output_layer.delta, 1);
    fill_cpu16(l.outputs * l.batch * l.steps, 0, self_layer.delta, 1);
    fill_cpu16(l.outputs * l.batch * l.steps, 0, input_layer.delta, 1);

    for (i = 0; i < l.steps; ++i) {
        s.input = net.input;        
	forward_connected_layer16(input_layer, s);

        s.input = l.state;
        forward_connected_layer16(self_layer, s);

        FLT *old_state = l.state;
        if(l.shortcut){
            copy_cpu16(l.outputs * l.batch, old_state, 1, l.state, 1);
        }else{
            fill_cpu16(l.outputs * l.batch, 0, l.state, 1);
        }
        axpy_cpu16(l.outputs * l.batch, 1, input_layer.output, 1, l.state, 1);
        axpy_cpu16(l.outputs * l.batch, 1, self_layer.output, 1, l.state, 1);

        s.input = l.state;
        forward_connected_layer16(output_layer, s);

        net.input += l.inputs*l.batch;
        increment_layer16(&input_layer, 1);
        increment_layer16(&self_layer, 1);
        increment_layer16(&output_layer, 1);
    }
}
