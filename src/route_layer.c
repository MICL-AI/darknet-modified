...
route_layer16 make_route_layer16(int batch, int n, int *input_layers, int *input_sizes)
{
    fprintf(stderr,"route16 ");
    route_layer16 l = {0};
    l.type = ROUTE;
    l.batch = batch;
    l.n = n;
    ...
    l.forward = forward_route_layer16;
    //l.backward = backward_route_layer;
    #ifdef GPU
    l.forward_gpu = forward_route_layer_gpu;
    l.backward_gpu = backward_route_layer_gpu;
...
void forward_route_layer16(const route_layer16 l, network16 net)
{
    int i, j;
    int offset = 0;
    for(i = 0; i < l.n; ++i){
        int index = l.input_layers[i];
        FLT  *input = net.layers[index].output;
        int input_size = l.input_sizes[i];
        for(j = 0; j < l.batch; ++j){
            copy_cpu16(input_size, input + j*input_size, 1, l.output + offset + j*l.outputs, 1);
        }
        offset += input_size;
    }
}
