...
/* for sparse mat */
#define val(a) ((a).val)               //get val in csr
...
spa_mat mat2csr_partation(val_t *a, count_t src_row, count_t src_col, count_t dst_row, count_t dst_col, count_t index_row, count_t index_col)
{
    ...
    count_t num_row = dst_row, num_col = dst_col;
    //solve corner cases
    ...
    if (index_row == part_row - 1 && index_col < part_col - 1) //bottom (south)
        num_row = (src_row % dst_row) ? (src_row % dst_row) : dst_row;
    else if (index_row < part_row - 1 && index_col == part_col - 1) //right (east)
        num_col = (src_col % dst_col) ? (src_col % dst_col) : dst_col;
    ...
    count_t num_nzd = 0;
    /*count number of none zero data*/
    for (count_t i = start_row; i < start_row + num_row; i++)
        for (count_t j = start_col; j < start_col + num_col; j++)
        {
            if (*(a + i * src_col + j))
            {
                num_nzd += 1;
...
int main(int argc, char const *argv[])
{
    ...
    // spmt = mat2csr_divide(mat_norm, num_row, num_col, 1, 4);
    ...
    for (int kr = 1; kr < 8; kr++)
        for (int kc = 1; kc < 8; kc++)
        {
            ...
            csr = csr2mat_comb(spmt);
            num_err += valid_mat(mat_norm, csr, num_row, num_col);
        }
    if (!num_err)
        puts("-----\nEvery single one goes right!\n-----");
    else
        printf("Err exists, total number = %d", num_err);
